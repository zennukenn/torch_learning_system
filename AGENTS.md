...
- Follow the C/R/S depth and capacity rules in `projects/ROADMAP.md`. Pass M0a quickly, require M0b before M2 and M0c before M3, and complete M2.5 CPU model inference before CUDA. Run `templates/SCOPE_REVIEW.md` every four active weeks or 20 substantive sessions.
...
- Prefer one validated session manifest and `scripts/record_learning_session.py` over hand-editing the same session into many state files. The manifest automates recording only after the learner has completed the required evidence and teach-back.
